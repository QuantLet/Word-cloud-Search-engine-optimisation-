...
class SearchEngine:
    def __init__(self, search_driver: IndexDriver):
        ...
        self.stopwords_list = {"i", "me", "my", "myself", "we", "our", "ours", "ourselves", "you", "you're", "you've",
    ...
    def index(self, courselet_id, pdf_url):
        data, duration = PDFManager(pdf_url).get_search_data()
        indexed = self.search_driver.index("courselets", courselet_id, data)
        if indexed:
            return True, {"id": courselet_id, "search_data": data, "duration": duration}
        else:
            return False, None
